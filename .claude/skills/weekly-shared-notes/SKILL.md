...
## 貼り付け先（週報メモの書式はこれに合わせる）

週報メモの「6. お客様の声・フィードバック」は、片山が毎週そのまま
**「HAWK営業 - 定例会報告書」にコピペする**運用（2026-09-02 決定）。

- 貼り付け先 Doc: `documentId: 1I7u2zNZTPl9tAo2SGalo35ElsDWu2MPpP6JgIYr8LCk`
  （タブは週ごとに `YYYYMMDD` 名で増える。貼るのは片山が手動で行う。
  **この Doc にこちらから書き込まない**——チーム共有 Doc のため）
- したがって週報メモ側は「そのまま貼れるプレーンテキスト」で書く:
