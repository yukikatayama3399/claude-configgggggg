...
- **書式を壊さない（2026-09-02 片山が手動整形済み）**: タイトル＝太字16pt、
  サマリー上側の━━線と `■ ニーズ集計サマリー` 行＝太字、サマリー内の
  `【〇〇の部】`＝太字、それ以外は標準。ページ設定（A4・余白）も固定。
  **全文削除→再挿入は禁止**、対象行のみのピンポイント編集
  （insertText／deleteContentRange）とする。挿入行は挿入位置の書式を継承
  するので、意図と違えば updateTextStyle で bold を明示（通常行= false、
  カテゴリ見出し= true）。書き込み後に読み返して太字の崩れを確認する。
- **新規追加・社数更新の行には行頭に【MM/DD】（追加日）を付ける**
  （サマリー・明細・週次アップデート共通。何が新しいか一目で分かるように）。
- **2社以上の声は週報にも載せる**: 週報メモ t.0 セクション6の
  `■ 顧客要望サマリー（2社以上の声・週報共有用）` ブロックを、
  本Docのサマリーから2社以上の項目のみ抜粋（カテゴリ見出し付き）で差し替える。
  ミラー（4c）はこのブロックをセクション6ごと保全する。
- 更新タイミングは2つ（どちらも既存内容を読んでから・重複追記禁止）:
  月曜 Routine「『顧客からの要望まとめ』週次更新」（全チャネル横断の本スイープ＋2社以上ブロック差し替え）と、
  水曜 HAWK週報 Routine の手順 4d（週報作成中に見つけた分の反映＋週報下書きへの転記）。
