...
## 機能要望ストック（t.1tg9z5jlttjh）は正の字方式

「こういうことをやってほしい」という要望ごとに **該当クライアントを（社名、社名…）で
列挙し、社数を付けて積み上げる**（正の字方式）。社数が多い＝ニーズが強い要望として
開発チームへ還元する。タブ先頭の
`■ ニーズ集計サマリー（要望 × 該当クライアント。社数の多い順）` を常に最新に保つ
（1行 = `・要望名（社名、社名、…）… N社`、多い順）。月曜の Routine に組込済み。
