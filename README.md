...
| `gmail/followup_report.py` | どこでも | メールの追いかけリスト（要返信 / 未返信 / 返信待ち）を出す（**読み取り専用**） |

### メールの追いかけリスト

送ったのに返ってこない／返ってきたのに返せていないスレッドだけを抜き出す。
一斉送信（同じ件名が大量にあるもの）とカレンダー通知・配信不能通知は自動で除外する。

```bash
python3 gmail/followup_report.py --days 60            # Markdown の表で出す
python3 gmail/followup_report.py --days 60 --json     # スクリプトから使う
```

状態は4つに分類する。差出人が `fout.jp` なら「自社が返した」と見なすので、
同席者（高橋・福浦 等）が引き取っているスレッドは要返信に出ない。

| 状態 | 意味 |
|---|---|
| `要返信` | 相手が最後に送っていて、こちらが返していない。**最優先** |
| `配信不能` | メールアドレスが死んでいる。宛先を直すまで何を送っても届かない |
| `未返信` | こちらが送ったが一度も返信が来ていない |
| `返信待ち` | やり取りはあるが、こちらが最後。相手のボール |

主なオプション:

- `--days N` … 遡る日数（既定 60）
- `--min-age N` … 最終やり取りから N 日以上経ったものだけ出す（既定 3）
- `--bulk-threshold N` … 同じ件名が N 件以上あれば一斉送信として除外（既定 10）。
  下げすぎると「ご検討状況のお伺い」のような**個別に送った定型文まで消える**ので注意

**送信も下書き作成もしない。** 読むだけなのでいつ叩いても安全。
