...
## HAWK 初回メールは「確定版テンプレ」を参照する

資料DL / インバウンドリードへの初回アプローチメールを書くときは、
必ず **`HAWK_初回メールテンプレ_確定版.md`** を読んでから書くこと。
件名・本文の改行の入れ方・87%削減の実績一文・パートナー制度の段落・署名まで
すべてそこが正。

スキル `hawk-inbound-lead` の `references/templates.md` にも初回メールの骨子があるが、
**内容が食い違う場合は `HAWK_初回メールテンプレ_確定版.md` を優先する**
（2026-08-12 に本人が Gmail 上で推敲・確定した版のため）。

送信禁止のルールは変わらない。**下書きまでで必ず止める**。
