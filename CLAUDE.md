...
打ち合わせの議事録は **1打ち合わせ = 1 Doc** を
`10_顧客/<NN>_<会社名>/1_議事録/` に追加していく（過去分は書き換えない）。
命名は `議事録_MMDD_<会社略称>様_<議題>`。

手順・本文フォーマット・「社外の方が退出したあとの社内会話」の扱いは
`.claude/skills/hawk-meeting-minutes/SKILL.md` に定義済み。
文字起こしURLを貼られたら（明示依頼が無くても）このスキルを使う。

### gog が使えないセッションでの Doc 書き込み

`GOG_CREDENTIALS_B64` 等が未設定のセッションでは gog が立ち上がらない
（`setup_gog_remote.sh` が「環境変数 GOG_CREDENTIALS_B64 が未設定」で止まる）。
その場合の代替として、**Drive MCP の `create_file` に
`contentMimeType: text/markdown` を指定すると Google Doc に変換されて
太字・見出し・ハイパーリンクが実体として入る**（2026-08-18 実測）。

- `text/plain` だと `**` がそのまま本文に残るので使わない。
- ただし **既存 Doc への追記は MCP ではできない**（`update_file` はタイトルと親のみ）。
  追記が必要なタスクは gog が使える環境で実行する。
  → 議事録を「1回=1Doc」設計にしているのはこの制約と噛み合わせるため。
